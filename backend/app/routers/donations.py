...
def delete_donation(donation_id: int, actor_email: str | None = None, db: Session = Depends(get_db)):
    deleted, error = donations_service.delete_by_id(db, donation_id, actor_email)
    if not deleted:
        if error == "forbidden":
